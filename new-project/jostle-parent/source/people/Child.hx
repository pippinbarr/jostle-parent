...
class Child extends Person
{
	...
	public function new(X:Float, Y:Float, ui:FlxGroup, Name:String, Number:Int, Color:Int, Controllable:Bool = false)
	{
		...
		// Set up wandering
		wanderTimer = new FlxTimer().start().start();
		wanderTimer.finished = true;
		wanderDirection = NONE;

		// Set up pausing
		pauseTimer = new FlxTimer().start().start();
		pauseTimer.finished = true;

		// Set up chasing
		chaseWanderTimer = new FlxTimer().start().start();

		// Set up door timer
		postDoorTimer = new FlxTimer().start().start();

		// Set up wander follow
		wanderFollowTimer = new FlxTimer().start().start();
		// currentWanderFollow = FOLLOW;

		dyingTimer = new FlxTimer().start().start();
		...
		if (!controllable)
			...
			MOVEMENT_IMPULSE = NORMAL_SPEED * 4;
	...
	public function showName():Void
	{
		...
		new FlxTimer().start().start(10, hideName);
	}
	...
	private function updateMovement():Void
	{
		...
		if (watchingTV && movementMode != JITTER)
			return;
	...
	override private function handleImpact(Impact:Int, OtherSprite:PhysicsSprite):Void
	{
		...
		switch (OtherSprite.kind)
		{
			...
			case POISON:
				if (Math.random() < CHANCE_OF_POISONING)
					startDying(POISONING);

			// If it's food then eat it (if not dying)
			case FOOD:
				if (dying)
					return;
				if (hungry && !watchingTV)
					eat(OtherSprite);

			// If it's the parent then they can affect movement and choking
			case PARENT:
				if (!dying && Impact > Physics.ANY)
					setMovementMode(WANDER_FOLLOW);
				if (state == CHOKING && Impact > Physics.MEDIUM)
					stopDying(CHOKING);
	...
	private function wander():Void
	{
		if (!wanderTimer.finished)
		{
			...
			if (wanderDirection == LEFT)
				canContinue = moveLeft();
			else if (wanderDirection == RIGHT)
				canContinue = moveRight();
			else if (wanderDirection == UP)
				canContinue = moveUp();
			else if (wanderDirection == DOWN)
				canContinue = moveDown();
			else if (wanderDirection == NONE)
				canContinue = idle();
		...
		else
		{
			...
			while (!foundDir)
			{
				...
				if (wanderDirection == LEFT && leftBlocks > 0)
					continue;
				if (wanderDirection == RIGHT && rightBlocks > 0)
					continue;
				if (wanderDirection == DOWN && downBlocks > 0)
					continue;
				if (wanderDirection == UP && upBlocks > 0)
					continue;
	...
	private function follow():Void
	{
		...
		if (target.place != place)
		{
			...
			if (currentTarget == null)
			{
				if (place.doors.length == 0)
					...
					currentTarget = (place.doors[Math.floor(Math.random() * place.doors.length)]);
			}
		}
		...
		if (Math.abs(lastFollowX - x) < 2 && Math.abs(lastFollowY - y) < 2)
		{
			staticFollowFrames++;
		}
		else
		...
		if (Math.abs((hit.x + hit.width / 2) - (target.x + target.width / 2)) > Math.abs((hit.y + hit.height / 2) - (target.y + target.height / 2)))
		{
			...
			if (!success)
				followY(currentTarget, false);
		}
		else
		{
			success = followY(currentTarget, false);
			if (!success)
				followX(currentTarget, false);
		}
	}
	...
	private function chase():Void
	{
		...
		if (Math.abs(lastChaseX - x) < 2 && Math.abs(lastChaseY - y) < 2)
		{
			staticChaseFrames++;
		}
		else
		...
		if (Math.abs((hit.x + hit.width / 2) - (target.hit.x + target.hit.width / 2)) > Math.abs((hit.y + hit.height / 2)
