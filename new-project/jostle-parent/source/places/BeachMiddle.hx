...
class BeachMiddle extends Place
{
	...
	public function new(bg:FlxGroup, display:FlxTypedGroup<Sortable>, Doors:FlxGroup, DoorMovables:FlxGroup):Void
	{
		...
		var rightFence:PhysicsSprite = new PhysicsSprite(OFFSET_X + WIDTH - leftFence.width, OFFSET_Y + HEIGHT - 3 * 8, AssetPaths.beach_right_fence__png, 1,
			0.2, 1, false);

		toFrontOfHouse = new Door(leftFence.x
			+ leftFence.width, leftFence.y
			+ 2 * 8, this, new FlxPoint(-1, leftFence.y - 2 * 8),
			"BeachMiddle.toFrontOfHouse", DOWN, true, rightFence.x
			- leftFence.x
			- leftFence.width, 1 * 8);
		...
		var leftUmbrella:PhysicsSprite = new PhysicsSprite(OFFSET_X + 11 * 8, OFFSET_Y + 33 * 8, AssetPaths.beach_umbrella__png, 0.1, 0.1, 1, false, false,
			false, 1, 1, false, 0, 0, "", 8, true);
		...
		var rightUmbrella:PhysicsSprite = new PhysicsSprite(OFFSET_X + 50 * 8, OFFSET_Y + 33 * 8, AssetPaths.beach_umbrella__png, 0.1, 0.1, 1, false, false,
			false, 1, 1, false, 0, 0, "", 8, true);
