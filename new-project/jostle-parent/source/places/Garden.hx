...
class Garden extends Place
{
	...
	public function new(bg:FlxGroup, display:FlxTypedGroup<Sortable>, Doors:FlxGroup, DoorMovables:FlxGroup):Void
	{
		...
		var upperLeftWall:PhysicsSprite = new PhysicsSprite(Std.int(topWall.x), Std.int(topWall.y + topWall.height), AssetPaths.garden_upper_left_wall__png,
			1, 1, 1000, false);
		var middleWall:PhysicsSprite = new PhysicsSprite(OFFSET_X, OFFSET_Y + 31 * 8, AssetPaths.garden_middle_wall__png, 1, 1, 1000, false);
		var lowerLeftWall:PhysicsSprite = new PhysicsSprite(Std.int(middleWall.x + middleWall.width - 2 * 8), Std.int(middleWall.y + middleWall.height),
			AssetPaths.garden_lower_left_wall__png, 1, 1, 1000, false);
		var bottomWallLeft:PhysicsSprite = new PhysicsSprite(Std.int(lowerLeftWall.x), Std.int(lowerLeftWall.y + lowerLeftWall.height),
			AssetPaths.garden_bottom_left_wall__png, 1, 1, 1000, false);
		var bottomWallRight:PhysicsSprite = new PhysicsSprite(Std.int(bottomWallLeft.x + bottomWallLeft.width) + Door.STANDARD_DOOR_WIDTH,
			Std.int(bottomWallLeft.y), AssetPaths.garden_bottom_right_wall__png, 1, 1, 1000, false);
		var rightWall:PhysicsSprite = new PhysicsSprite(Std.int(topWall.x + topWall.width) - 2 * 8, Std.int(topWall.y + topWall.height),
			AssetPaths.garden_right_wall__png, 1, 1, 1000, false);

		toLivingRoom = new Door(bottomWallLeft.x + bottomWallLeft.width, bottomWallLeft.y + 2 * 8, this, new FlxPoint(-1, bottomWallLeft.y - 2 * 8),
			"Garden.toLivingRoom", DOWN);
		...
		var grass2:FlxSprite = new FlxSprite(Std.int(middleWall.x + middleWall.width), Std.int(grass1.y + grass1.height),
			AssetPaths.garden_mowed_grass_tile__png);
		...
		{
			lawnmower = new PhysicsSprite(FlxG.save.data.garden_lawnmower.x, FlxG.save.data.garden_lawnmower.y, AssetPaths.garden_lawnmower__png, 1, 0.3, 0.5,
				true, false, false, 10, 1, false, 0, 0, "", 8, false, Physics.PARENT, (Physics.SOLID | Physics.PARENT | Physics.PARENT_LOCK));

			poison = new PhysicsSprite(FlxG.save.data.garden_poison.x, FlxG.save.data.garden_poison.y, AssetPaths.garden_poison__png, 1, 0.25, 8, true, false,
				true);
		}
		else
		{
			lawnmower = new PhysicsSprite(OFFSET_X + 32 * 8, OFFSET_Y + 24 * 8, AssetPaths.garden_lawnmower__png, 1, 0.3, 0.5, true, false, false, 1, 1,
				false, 0, 0, "", 8, false, Physics.PARENT, (Physics.SOLID | Physics.PARENT | Physics.PARENT_LOCK));

			poison = new PhysicsSprite(OFFSET_X + 3 * 8, OFFSET_Y + 24 * 8, AssetPaths.garden_poison__png, 1, 0.25, 8, true, false, true);
		}
	...
	public function autoMow():Void
	{
		...
		unmowedGrass.forEach(function(grass:FlxBasic)
		{
			grass.kill();
		});
	}
