...
class Menu extends Place
{
	...
	public function new(bg:FlxGroup, display:FlxTypedGroup<Sortable>, Doors:FlxGroup, DoorMovables:FlxGroup):Void
	{
		...
		for (i in 0...titleStringUpper.length)
		{
			...
			{
				l = new PhysicsSprite(FlxG.save.data.menu_letters[letterIndex + i].x, FlxG.save.data.menu_letters[letterIndex + i].y,
					"assets/images/menu/menu_" + titleStringUpper.charAt(i).toLowerCase() + ".png", 1, 0.25, 1, true, false, false, 2, 1, false, 0, 0, "", 8);
			}
			else
			{
				l = new PhysicsSprite(letterX, letterY, "assets/images/menu/menu_" + titleStringUpper.charAt(i).toLowerCase() + ".png", 1, 0.25, 1, true,
					false, false, 2, 1, false, 0, 0, "", 8);
			}
			letters.push(l);
		...
		for (i in 0...titleStringLower.length)
		{
			...
			{
				l = new PhysicsSprite(FlxG.save.data.menu_letters[letterIndex + i].x, FlxG.save.data.menu_letters[letterIndex + i].y,
					"assets/images/menu/menu_" + titleStringLower.charAt(i).toLowerCase() + ".png", 1, 0.25, 1, true, false, false, 2, 1, false, 0, 0, "", 8);
			}
			else
			{
				l = new PhysicsSprite(letterX, letterY, "assets/images/menu/menu_" + titleStringLower.charAt(i).toLowerCase() + ".png", 1, 0.25, 1, true,
					false, false, 2, 1, false, 0, 0, "", 8);
			}
			letters.push(l);
			letterX = l.x + l.width + 10;
