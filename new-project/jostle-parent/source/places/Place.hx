...
class Place
{
	...
	public function contains(S:PhysicsSprite):Bool
	{
		return !S.hidden
			&& S.alive
			&& !(S.x > ORIGIN_X + FlxG.width || S.x + S.width < ORIGIN_X || S.y > ORIGIN_Y + FlxG.height || S.y + S.height < ORIGIN_Y);
	}
	...
	public function handlePlaceDoors(P:PhysicsSprite):Bool
	{
		...
		if (P.place != this || !P.alive || !P.active)
			return false;

		for (i in 0...doors.length)
		{
			if (!moved)
				moved = doors[i].handleOverlap(P);
		}
