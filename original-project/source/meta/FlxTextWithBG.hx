...
class FlxTextWithBG extends FlxGroup
{
	...
	public var velocityX(get_velocityX,set_velocityX):Float;
	...
	public function new(
		X:Float,Y:Float,W:Float,T:String,S:Int,A:String,TextColor:UInt,BGColor:UInt,
		Tweens:Bool=false,TweenDir:Enums.Direction = null,Alpha:Float = 0.6,Indent:Int = 8)
	{
		super();
	...
	{
		if (!tweens) return;
		if (S == text.text && !isTweenedOut()) return;
	...
	override public function update():Void
	{
		...
		switch (tweenState)
		{
			...
			case TWEEN_IN_DELAY:
			if (tweenInTimer.finished) tweenState = TWEENED_IN;
