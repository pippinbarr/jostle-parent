...
class Music extends FlxBasic
{
	...
	private static var TRACKS:Array<Array<String>> = 
	...
	override public function update():Void
	{
		...
		if (PlayState.justJostled != NONE && frames == FRAMES_PER_BEAT)
		{
			...
			if (PlayState.justJostled == NORMAL) jostle.play(true);
			else if (PlayState.justJostled == SOFT) softJostle.play(true);
		...
		if (frames >= FRAMES_PER_BEAT)
		{
			...
			if (track[index].indexOf("k") != -1) kicks[Math.floor(Math.random() * kicks.length)].play(true);
			...
			if (track[index].indexOf("h") != -1) hihats[Math.floor(Math.random() * hihats.length)].play(true);
			if (track[index].indexOf("p") != -1) splashes[Math.floor(Math.random() * splashes.length)].play(true);
	...
	private function setVolumes():Void
	{
		...
		for (i in 0...kicks.length) kicks[i].volume = volume;
		for (i in 0...snares.length) snares[i].volume = volume;
		for (i in 0...hihats.length) hihats[i].volume = volume;
		for (i in 0...splashes.length) splashes[i].volume = volume;
	}
	...
	override public function destroy():Void
	{
		...
		jostle.destroy();
		softJostle.destroy();
		for (i in 0...kicks.length) kicks[i].destroy();
		for (i in 0...snares.length) snares[i].destroy();
		for (i in 0...hihats.length) hihats[i].destroy();
		for (i in 0...splashes.length) splashes[i].destroy();
	}
