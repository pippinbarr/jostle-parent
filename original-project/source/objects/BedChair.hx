...
class BedChair extends PhysicsSprite
{
	...
	public function new (
		...
		T:String = "",
		Scale:Float = -1,
		CenterHitBox:Bool = false,
		CategoryBits:Int = -1,
		MaskBits:Int = -1,
		ThePlace:Place = null,
		TheKind:Enums.Kind = null)
	{
		super(X,Y,Sprite,HitXRatio,HitYRatio,Mass,IsDynamic,HasSensor,Ignore,Friction,Restitution,Animated,W,H,T,Scale,CenterHitBox,CategoryBits,MaskBits);

		place = ThePlace;
		kind = TheKind;

		useTimer = new FlxTimer();
		useTimer.start(1);
	...
	public function setupAnimation(
		EmptyFrames:Array<Int>,
		OccupiedFrames:Array<Int>,
		Child1Frames:Array<Int>,
		Child2Frames:Array<Int>,
		Child3Frames:Array<Int>,
		BlackColour:Int = -1,
		RedColour:Int = -1,
		CyanColour:Int = -1,
		BlueColour:Int = -1):Void
	{
		...
		replaceColor(0xFF000000,BlackColour);
		replaceColor(0xFFFF0000,RedColour);
		replaceColor(0xFF00FFFF,CyanColour);
		replaceColor(0xFF0000FF,BlueColour);
	}
	...
	{
		if (c.statusText.text.text == c.name) return;
	...
	{
		if (user == null) return;
		if (!useTimer.finished && useTimer.elapsedTime < 2) return;
