...
{	
	...
	public function new (X:Float,Y:Float,P:Place,Arrival:FlxPoint,Name:String,Orientation:Enums.Direction,Hides:Bool = false,W:Float = 12*8,H:Float = 1*8)
	{
	...
	{
		if (o.kind == PARENT && cast(o,Parent).isDead()) return false;
		...
		{			
			...
			if (o.kind == PARENT) FlxG.camera.focusOn(toDoor.place.FOCUS_POINT);
			if (o.kind == CHILD) cast(o,Child).setMovementMode(POST_DOOR_STILL);
