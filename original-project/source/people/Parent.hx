...
class Parent extends Person
{
	...
	public function new(X:Float,Y:Float,C1:Child,C2:Child,C3:Child,C:Int)
	{
		...
		replaceColor(0xFFFF0000,child1.childColor);
		replaceColor(0xFF00FFFF,child2.childColor);
		replaceColor(0xFF0000FF,child3.childColor);
		replaceColor(0xFF000000,parentColor);
		...
		drowningTimer = new FlxTimer();

		hugger = null;
		hugTimer = new FlxTimer();
	...
	{			
		if (FlxG.keys.pressed.ALT || FlxG.keys.pressed.CONTROL || FlxG.keys.pressed.SHIFT) FlxG.keys.reset();

		if (hugger != null && hugTimer.elapsedTime < 1) return;
		if (hugger != null && FlxG.keys.pressed.ANY) hugTimerFinished(null);
	...
	override private function handleImpact(Impact:Int,OtherSprite:PhysicsSprite):Void
	{
		...
		{
			case BALL:
			if (playingBall) playingBallTouches++;

			case CHILD:
			if (playingBall) playingBallTouches++;
			if (hugger == null && hugTimer.finished) 
			{
				if (place.location == BEACH) return; // No hugging at the beach (too weird in the water)
				if (Math.abs(x - OtherSprite.x) < 8 && y < OtherSprite.y) hug(cast(OtherSprite,Child));
			}
	...
	{
		if (hugger == null) return;

		if (child1.dying || child2.dying || child3.dying)
		{
			hugger.unhide();
			if (!dead) animation.play("normal");
		}
	}
