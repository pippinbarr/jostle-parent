...
class Bedroom extends Place
{
	...
	{		
		...
		{
			dresser = new PhysicsSprite(
				FlxG.save.data.bedroom_dresser.x,
				FlxG.save.data.bedroom_dresser.y,
				AssetPaths.bedroom_dresser__png,
		...
		bed = new BedChair(
			bed_x,
			bed_y,
			AssetPaths.bedroom_bed__png,
			1,0.25,40,true,false,false,1,1,true,15,13,"",8,false,Physics.SOLID,Physics.SOLID|Physics.PARENT,this,BED);
		bed.setupAnimation([0,0],[1,1],[2,3],[4,5],[6,7],
			PlayState.parent.parentColor,PlayState.child1.childColor,PlayState.child2.childColor,PlayState.child3.childColor);
		...
		var toilet:PhysicsSprite = new PhysicsSprite(OFFSET_X + WIDTH - 8*8,OFFSET_Y + HEIGHT - 9*8,AssetPaths.bedroom_toilet__png,1,0.25,10,false,false);
		...
		var shower:PhysicsSprite = new PhysicsSprite(ensuite_left.x + ensuite_left.width,OFFSET_Y + 2*8,AssetPaths.bedroom_shower__png,1,1,10,false,false);
		...
		var ensuite_right:PhysicsSprite = new PhysicsSprite(shower.x + shower.width,OFFSET_Y + 2*8,AssetPaths.bedroom_ensuite_right__png,1,1,10,false,false);
	...
	override public function update():Void
	{
		...
		if (parent.place == this && showerIsRunning()) showerSound.play();
		else showerSound.stop();
	...
	{
		if (!showerTimer.finished) return;
