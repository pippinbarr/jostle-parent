...
class Park extends Place
{
	...
	public function new(bg:FlxGroup, display:FlxTypedGroup<Sortable>,Doors:FlxGroup,DoorMovables:FlxGroup):Void
	{
		...
		var bottomRightWall:Wall = new Wall(Std.int(bottomLeftWall.x + bottomLeftWall.width + DOOR_WIDTH),OFFSET_Y + HEIGHT - 8,Std.int((WIDTH - DOOR_WIDTH)/2),8,AssetPaths.park_wall_tile__png);

		toBeach = new Door(bottomLeftWall.x + bottomLeftWall.width,bottomLeftWall.y + 2*8,this,new FlxPoint(-1,bottomLeftWall.y - 2*8),"Park.toBeach",DOWN,true);
		doors.push(toBeach);
		...
		var rightFlowers:PhysicsSprite = new PhysicsSprite(OFFSET_X + WIDTH - leftFlowers.width - 8,OFFSET_Y + 8,AssetPaths.park_right_flowers__png,1,1,1,false);
