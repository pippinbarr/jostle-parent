...
class Playground extends Place
{
	...
	public function new(bg:FlxGroup, display:FlxTypedGroup<Sortable>,Doors:FlxGroup,DoorMovables:FlxGroup):Void
	{
		...
		var bottomRightWall:Wall = new Wall(Std.int(bottomLeftWall.x + bottomLeftWall.width + DOOR_WIDTH),OFFSET_Y + HEIGHT - 8,Std.int((WIDTH - DOOR_WIDTH)/2),8,AssetPaths.playground_wall_tile__png);
		...
		swing = new PhysicsSprite(OFFSET_X + 50*8,OFFSET_Y + 6*8,AssetPaths.playground_swing__png,0.65,0.2,1,false,false,false,1,1,true,17,16,"",8,true);
		...
		seesaw = new PhysicsSprite(OFFSET_X + 46*8,OFFSET_Y + 32*8,AssetPaths.playground_seesaw__png,1,0.2,1,false,false,false,1,1,true,19,13);
		...
		springRider = new PhysicsSprite(OFFSET_X + 12*8,OFFSET_Y + 32*8,AssetPaths.playground_spring_rider__png,0.6,0.2,1,false,false,false,1,1,true,8,12,"",8,true);
	...
	{
		if (PlayState.parent.place != this) return;
		...
		{
			if (!contains(PlayState.child1) && PlayState.child1.alive) PlayState.child1.die("got lost");
			if (!contains(PlayState.child2) && PlayState.child2.alive) PlayState.child2.die("got lost");
			if (!contains(PlayState.child3) && PlayState.child3.alive) PlayState.child3.die("got lost");
		}
	...
	public function checkSlide(c:Child,p:Parent):Void
	{
		...
		if (c.hit.overlaps(slideTrigger) && slider == null)
		{
			...
			slide.origin.x = 0; slide.origin.y = 0;
	...
	public function checkSpringRider(c:Child,p:Parent):Void
	{
		...
		if (c.hit.overlaps(springRiderTrigger) && springRiderer == null && springRider.active)
		{
			...
			springRider.origin.x = 0; springRider.origin.y = 0;
			springRider.animation.add("empty",[0,0],1,false);
			springRider.animation.add("occupied",[1,1],1,false);
			springRider.animation.add("springing",[1,2,3,4],10,true);
			...
			new FlxTimer(2 + Math.random()*2,dismountSpringRider);
		}
	...
	public function checkSwing(c:Child,p:Parent):Void
	{
		...
		if (c.hit.overlaps(swingTrigger) && swinger == null && swing.active)
		{
			...
			swing.origin.x = 0; swing.origin.y = 0;
			...
			new FlxTimer(10 + Math.random()*10,dismountSwing);
		}
	...
	public function checkSeesaw(c:Child,p:Parent):Void
	{
		...
		if (c.hit.overlaps(seesawLeftTrigger) && leftSeesawer == null && seesaw.active)
		{
			...
			seesaw.origin.x = 0; seesaw.origin.y = 0;
			seesaw.animation.add("empty",[0,0],1,false);
			seesaw.animation.add("left occupied",[4,4],1,false);
			seesaw.animation.add("right occupied",[5,5],1,false);
			seesaw.animation.add("seesawing",[2,1,3,1],10,true);

			if (leftSeesawer != null) seesaw.replaceColor(0xFF000000,leftSeesawer.childColor);
			if (rightSeesawer != null) seesaw.replaceColor(0xFFFFFFFF,rightSeesawer.childColor);
			...
			new FlxTimer(10 + Math.random()*10,dismountLeftSeesaw);
		}
		...
		if (c.hit.overlaps(seesawRightTrigger) && rightSeesawer == null && seesaw.active)
		{
			...
			new FlxTimer(10 + Math.random()*10,dismountRightSeesaw);
		}
	...
	{
		if (currentWatchedEquipment == seesaw || leftSeesawer == null) return; // Can't get off if parent is watching (until done)
		...
		if (rightSeesawer != null) seesaw.animation.play("right occupied");
		else seesaw.animation.play("empty");

		seesaw.active = false;

		new FlxTimer(5,activateSeesaw);

		if (latestMountedEquipment == seesaw && rightSeesawer == null) latestMountedEquipment = null;
	}


	private function dismountRightSeesaw(t:FlxTimer):Void
	{
		if (currentWatchedEquipment == seesaw || rightSeesawer == null) return;
		...
		if (leftSeesawer != null) seesaw.animation.play("left occupied");
		else seesaw.animation.play("empty");
		seesaw.active = false;

		new FlxTimer(5,activateSeesaw);

		if (latestMountedEquipment == seesaw && leftSeesawer == null) latestMountedEquipment = null;
	}
	...
	{
		if (currentWatchedEquipment == springRider || springRiderer == null) return;
		...
		new FlxTimer(5,activateSpringRider);

		if (latestMountedEquipment == springRider) latestMountedEquipment = null;
	}
	...
	{
		if (currentWatchedEquipment == swing || swinger == null) return;
		...
		new FlxTimer(5,activateSwing);

		if (latestMountedEquipment == swing) latestMountedEquipment = null;
	}
