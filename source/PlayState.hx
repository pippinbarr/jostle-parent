...
class PlayState extends FlxState
{
	...
	"Jane","John","Sally","Jenny","Joey","Frank","Laila","Sue","Pippin",
	"Valerie","David","Peter","Louise","Gordon","Ana","Anne","Nicole","Alex",
	"Bettina","Rami","Costa","Elena","Davey","Leigh","Cara","Liz","Jenn",
	"Jim","Mary","George","Hector","Amy","Andy","Phil","Ashley"];
	...
	override public function create():Void
	{
		...
		childNames = flixel.util.FlxRandom.shuffleArray(childNames,1);

		var child1Name:String;
		if (FlxG.save.data.child1name != null) child1Name = FlxG.save.data.child1name;
		else child1Name = childNames.pop();
		...
		if (FlxG.save.data.child2name != null) child2Name = FlxG.save.data.child2name;
		else child2Name = childNames.pop();
		...
		if (FlxG.save.data.child1name != null) child3Name = FlxG.save.data.child3name;
		else child3Name = childNames.pop();
		...
		places = [bedroom,hallway,kidsRoom,bathroom,livingRoom,garden,frontOfHouse,playground,park,beachLeft,beachRight,beachMiddle,graveyard,cell,menu];
		...
		blackBG.scrollFactor.x = 0; blackBG.scrollFactor.y = 0;
		...
		if (state == BLACK_TO_MENU)
		{
			...
			if (child1.alive) living.push(child1);
			if (child2.alive) living.push(child2);
			if (child3.alive) living.push(child3);
			...
			if (FlxG.save.data.camera_focus_x == null) taskText.tweenIn("Move off-screen with the ARROW KEYS to begin playing.");
			...
			if (parent.isDead()) FlxG.camera.follow(parent);	
		}
		else if (state == PLAY)
		{
	...
	private function newWorld():Void
	{
		...
		if (FlxG.save.data.child1_alive != null) child1.alive = FlxG.save.data.child1_alive;
		if (FlxG.save.data.child1_alive != null) child2.alive = FlxG.save.data.child2_alive;
		if (FlxG.save.data.child1_alive != null) child3.alive = FlxG.save.data.child3_alive;



		if (task != GET_UP) bedroom.emptyBed();
	...
	private function loadWorld():Void
	{
		...
		if (FlxG.save.data.parentDead) parent.setDead();
		...
		if (Type.createEnum(Enums.Location,FlxG.save.data.parentLocation) != Enums.Location.MENU)
		{
			...
			if (task == MOW_THE_LAWN) garden.lock.lock();


			// Move the parent to where they were at the moment of saving
			parent.moveTo(FlxG.save.data.parentPosition.x,FlxG.save.data.parentPosition.y,locationToPlace(Type.createEnum(Enums.Location,FlxG.save.data.parentLocation)));

			// Move the children to where they were at the moment of saving
			child1.moveTo(FlxG.save.data.child1Position.x,FlxG.save.data.child1Position.y,locationToPlace(Type.createEnum(Enums.Location,FlxG.save.data.child1Location)));
			child2.moveTo(FlxG.save.data.child2Position.x,FlxG.save.data.child2Position.y,locationToPlace(Type.createEnum(Enums.Location,FlxG.save.data.child2Location)));
			child3.moveTo(FlxG.save.data.child3Position.x,FlxG.save.data.child3Position.y,locationToPlace(Type.createEnum(Enums.Location,FlxG.save.data.child3Location)));

			// Hide the children if they were hidden or dead
			if (FlxG.save.data.child1_hidden || !child1.alive) child1.hide();
			if (FlxG.save.data.child2_hidden || !child2.alive) child2.hide();
			if (FlxG.save.data.child3_hidden || !child3.alive) child3.hide();
			...
			if (task == GET_UP || task == TURN_ON_SHOWER || task == SHOWER || task == GO_TO_KIDS_ROOM || task == WAKE_KIDS || task == GO_TO_BED)
			{
				...
				if (child1.alive && FlxG.save.data.child1_in_bed) child1.setInBed(true);
				if (child2.alive && FlxG.save.data.child2_in_bed) child2.setInBed(true);
				if (child3.alive && FlxG.save.data.child3_in_bed) child3.setInBed(true);
			}
			...
			else
			{
				...
				{
					case MENU: FlxG.camera.focusOn(menu.FOCUS_POINT);
					case BEDROOM: FlxG.camera.focusOn(bedroom.FOCUS_POINT);
					case HALLWAY: FlxG.camera.focusOn(hallway.FOCUS_POINT);
					case KIDS_ROOM: FlxG.camera.focusOn(kidsRoom.FOCUS_POINT);
					case BATHROOM: FlxG.camera.focusOn(bathroom.FOCUS_POINT);
					case LIVING_ROOM: FlxG.camera.focusOn(livingRoom.FOCUS_POINT);
					case GARDEN: FlxG.camera.focusOn(garden.FOCUS_POINT);
					case FRONT_OF_HOUSE: FlxG.camera.focusOn(frontOfHouse.FOCUS_POINT);
					case PLAYGROUND: FlxG.camera.focusOn(playground.FOCUS_POINT);
					case PARK: FlxG.camera.focusOn(park.FOCUS_POINT);
					case BEACH: FlxG.camera.focusOn(beachMiddle.FOCUS_POINT);
					case GRAVEYARD: FlxG.camera.focusOn(graveyard.FOCUS_POINT);
					case CELL: FlxG.camera.focusOn(cell.FOCUS_POINT);
					case UNKNOWN: FlxG.camera.focusOn(new FlxPoint(-100,-100));
				}
			}
		}
		...
		if (FlxG.save.data.child1_dying) child1.startDying(Type.createEnum(Child.ChildState,FlxG.save.data.child1_dying_state));
		if (FlxG.save.data.child2_dying) child2.startDying(Type.createEnum(Child.ChildState,FlxG.save.data.child2_dying_state));
		if (FlxG.save.data.child3_dying) child3.startDying(Type.createEnum(Child.ChildState,FlxG.save.data.child3_dying_state));
	}
	...
	override public function update():Void
	{
		...
		switch (state)
		{
			...
			if (graveyard.checkParentGraveTrigger(parent))
			{
				...
				new FlxTimer(1,playFadedToGameOver);
			...
			if (taskText.isTweenedOut())
			{
				...
				if (!child1.alive) deadChildren.push(child1);
				if (!child2.alive) deadChildren.push(child2);
				if (!child3.alive) deadChildren.push(child3);
				...
				{
					taskText.tweenIn("Mourn " + deadChildren[0].name + ", " + deadChildren[1].name + ", and " + deadChildren[2].name + " and leave when you are ready.");
				}
			}
			graveyard.update();
	...
	private function updateTask():Void
	{
		...
		switch (task)
		{
					...
					FlxG.keys.pressed.RIGHT))
			{
				...
				if (numLivingChildren() > 0) task = TURN_ON_SHOWER;
				else task = GO_TO_KIDS_ROOM;
			}
			...
			{
				taskText.tweenOut();
				if ((child1.alive && child1.inBed) || (child2.alive && child2.inBed) || (child3.alive && child3.inBed)) task = GO_TO_KIDS_ROOM;
				else task = TAKE_KIDS_TO_LIVING_ROOM;
			}
			...
			{
				taskText.tweenOut();
				if (numLivingChildren() > 0) task = WAKE_KIDS;
				else task = GET_BREAKFAST_FROM_FRIDGE;
			}
			...
			if ((!child1.alive || child1.active) && 
				(!child2.alive || child2.active) && 
				(!child3.alive || child3.active) && 
				taskText.isTweenedIn())
			{
				taskText.tweenOut();
			...
			if (taskText.isTweenedOut())
			{
				if (numLivingChildren() > 0) taskText.tweenIn("Get breakfast from the refrigerator for the kids.");
				else  taskText.tweenIn("Get breakfast from the refrigerator.");
			...
			{
				taskText.tweenOut();
				if (numLivingChildren() > 0) task = FEED_KIDS_BREAKFAST;
				else task = TAKE_KIDS_TO_GARDEN;
			}
			...
			if (taskText.isTweenedOut())
			{
				if (numLivingChildren() > 0) taskText.tweenIn("Take the kids out into the garden.");
				else taskText.tweenIn("Go to the garden.");
			}
			...
			if (taskText.isTweenedOut())
			{
				if (numLivingChildren() > 0) taskText.tweenIn("Mow the lawn until you're satisfied, then take the kids back inside.");
				else taskText.tweenIn("Mow the lawn until you're satisfied, then go back inside.");
			}
			...
			if (taskText.isTweenedOut())
			{
				if (numLivingChildren() > 0) taskText.tweenIn("Take the kids out to the front of the house.");
				else taskText.tweenIn("Go out to the front of the house.");
			}
			...
			if (taskText.isTweenedOut())
			{
				if (numLivingChildren() > 0) taskText.tweenIn("Put the kids on the bus and get on.");
				else taskText.tweenIn("Get on the bus.");
			}
			...
			if (taskText.isTweenedOut())
			{
				if (numLivingChildren() > 0) taskText.tweenIn("Get the kids out of the playground.");
				else taskText.tweenIn("Leave the playground.");
			}

			if (!taskText.isTweenedIn()) return;

			if (!playground.contains(child1) &&
				!playground.contains(child2) &&
				!playground.contains(child3))
			{
			...
			if (numDeadChildren() == 3 ||
				parent.getPlayingBallTotal() > MIN_PLAYING_BALL_TOUCHES)
			{
				taskText.tweenOut();
				...
				if (numLivingChildren() > 0) task = FEED_KIDS_LUNCH;
				else task = LEAVE_PARK;
			}
			...
			if (taskText.isTweenedOut())
			{
				if (numLivingChildren() > 0 )taskText.tweenIn("Feed the kids lunch.");
			}
			...
			if (taskText.isTweenedOut())
			{
				if (numLivingChildren() > 0) taskText.tweenIn("Get the kids out of the park and leave.");
				else taskText.tweenIn("Leave the park.");
			}		

			if (!taskText.isTweenedIn()) return;

			if (!park.contains(child1) &&
				!park.contains(child2) &&
				!park.contains(child3))
			{
			...
			if (taskText.isTweenedOut())
			{
				if (numLivingChildren() > 0) taskText.tweenIn("Supervise the kids while they swim, but make sure you let them move around.");				
				else taskText.tweenIn("Remember.");
			}
			...
			if (taskText.isTweenedOut())
			{
				if (numLivingChildren() > 0) taskText.tweenIn("Get the kids off the beach and leave.");
				else taskText.tweenIn("Leave the beach.");
			}		

			if (!taskText.isTweenedIn()) return;


			if (!beachLeft.contains(child1) && !beachMiddle.contains(child1) && !beachRight.contains(child1) &&
				!beachLeft.contains(child2) && !beachMiddle.contains(child2) && !beachRight.contains(child2) &&
				!beachLeft.contains(child3) && !beachMiddle.contains(child3) && !beachRight.contains(child3))
			{
			...
			if (taskText.isTweenedOut())
			{
				if (numLivingChildren() > 0) taskText.tweenIn("Get the kids into the house.");
				else taskText.tweenIn("Go inside.");
				// frontOfHouse.resetBusNoPassengers();
			}		
			...
			if (taskText.isTweenedOut())
			{
				if (numLivingChildren() > 0) taskText.tweenIn("Get dinner from the refrigerator for the kids.");
				else taskText.tweenIn("Get dinner from the refrigerator.");
			...
			{
				taskText.tweenOut();
				if (numLivingChildren() > 0) task = FEED_KIDS_DINNER;
				else task = TAKE_KIDS_TO_BEDROOM;
			}
			...
			if (taskText.isTweenedOut())
			{
				if (numLivingChildren() > 0) taskText.tweenIn("Take the kids to their bedroom.");
				else taskText.tweenIn("Go to the kids' room.");
			}
				...
				kidsRoom.contains(parent))
			{
				...
				if (numLivingChildren() > 0) task = PUT_KIDS_TO_BED;
				else task = GO_TO_BED;
			}
	...
	private function updatePlaces():Void
	{
		...
		if (parent.place == beachMiddle)
		{
			if (!parent.isDead())
			{
				if (beachLeft.contains(parent)) FlxG.camera.focusOn(beachLeft.FOCUS_POINT);
				if (beachMiddle.contains(parent)) FlxG.camera.focusOn(beachMiddle.FOCUS_POINT);
				if (beachRight.contains(parent)) FlxG.camera.focusOn(beachRight.FOCUS_POINT);
			}
	...
	private function updateDeath():Void
	{
		...
		if (child1.justDied() || child2.justDied() || child3.justDied() || parent.justDied())
		{
			...
			if (child1.isDying() || child2.isDying() || child3.isDying() || parent.isDying()) return;
			...
			new FlxTimer(1,playFadedToDeathNotice);
	...
	private function playFadedToDeathNotice(t:FlxTimer):Void
	{
		...
		if (parent.justDied()) theDead.push(parent);
		if (child1.justDied()) theDead.push(child1);
		if (child2.justDied()) theDead.push(child2);
		if (child3.justDied()) theDead.push(child3);

		for (i in 0...theDead.length)
		{
			if (theDead[i].kind == PARENT) text += "You " + cast(theDead[i],Parent).getCauseOfDeath() + ".\n";
			else text += cast(theDead[i],Child).name + " " + cast(theDead[i],Child).getCauseOfDeath() + ".\n";
		}
		...
		new FlxTimer(5,deathNoticeTimerFinished);
	}
	...
	private function menuFadedToBlack():Void
	{
		...
		if (!parent.isDead())
		{
			...
			{
				parent.moveTo(bedroom.bed.hit.x + bedroom.bed.width - parent.width,bedroom.bed.hit.y + bedroom.bed.hit.height - parent.height + parent.hit.height,bedroom);
			}
			else
			{
			...
			new FlxTimer(2,alarmClockDelayTimer);
	...
	private function playFadedToGameOver(t:FlxTimer):Void
	{
		...
		if (!child1.alive) deadChildren.push(child1); else livingChildren.push(child1);
		if (!child2.alive) deadChildren.push(child2); else livingChildren.push(child2);
		if (!child3.alive) deadChildren.push(child3); else livingChildren.push(child3);
	...
	private function locationToPlace(L:Enums.Location):Place
	{
		switch (L)
		{
			...
			case KIDS_ROOM: return kidsRoom;
			case BATHROOM: return bathroom;
			case LIVING_ROOM: return livingRoom;
			case GARDEN: return garden;
			case FRONT_OF_HOUSE: return frontOfHouse;
			case PLAYGROUND: return playground;
			case PARK: return park;
			case BEACH: return beachMiddle;
