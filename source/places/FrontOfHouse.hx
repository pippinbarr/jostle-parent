...
class FrontOfHouse extends Place
{
	...
	public function new(bg:FlxGroup, display:FlxTypedGroup<Sortable>,Doors:FlxGroup,DoorMovables:FlxGroup,TheParent:Parent):Void
	{
		...
		var rightFence:PhysicsSprite = new PhysicsSprite(OFFSET_X + leftFence.width + DOOR_WIDTH, OFFSET_Y, AssetPaths.front_of_house_right_fence__png, 1, 1, 1, false);


		toLivingRoom = new Door(leftFence.x + leftFence.width,leftFence.y - 2*8,this,new FlxPoint(-1,leftFence.y + 2*8),"FrontOfHouse.toLivingRoom",UP);
		...
		roadAndPavement.width *= 8; roadAndPavement.height *= 8;

		var bottomWall:PhysicsSprite = new PhysicsSprite(OFFSET_X, roadAndPavement.y + roadAndPavement.height, AssetPaths.front_of_house_bottom_wall__png, 1, 1, 1, false);
		...
		var busStop:PhysicsSprite = new PhysicsSprite(
			OFFSET_X + 68.5*8,OFFSET_Y + 24*8,
			AssetPaths.front_of_house_bus_stop__png,
			0.1,0.1,1,false,false,false,1,1,false,0,0,"",8,true);


		bus = new PhysicsSprite(OFFSET_X + 31*8,OFFSET_Y + 21*8,AssetPaths.front_of_house_bus__png,1,0.25,1000,true,false,false,1,1,true,43,16);
	...
	override public function update():Void
	{
		...
		if (car2.body.getLinearVelocity().x == 0) car2Sound.stop();
		if (bus.body.getLinearVelocity().x == 0) busSound.stop();
		...
		if (PlayState.child1.alive) numLivingChildren++;
		if (PlayState.child2.alive) numLivingChildren++;
		if (PlayState.child3.alive) numLivingChildren++;

		if (PlayState.parent.place == this)
		{
			checkCrossing(PlayState.parent);
			if (PlayState.task == PUT_KIDS_ON_BUS) checkBus(PlayState.parent,numLivingChildren);
		}
		if (PlayState.child1.place == this)
		{
			checkCrossing(PlayState.child1);
			if (PlayState.task == PUT_KIDS_ON_BUS) checkBus(PlayState.child1,numLivingChildren);
			if (!contains(PlayState.child1)) PlayState.child1.die("got lost");
		}
		if (PlayState.child2.place == this)
		{
			checkCrossing(PlayState.child2);
			if (PlayState.task == PUT_KIDS_ON_BUS) checkBus(PlayState.child2,numLivingChildren);
			if (!contains(PlayState.child2)) PlayState.child2.die("got lost");
		}
		if (PlayState.child3.place == this)
		{
			checkCrossing(PlayState.child3);
			if (PlayState.task == PUT_KIDS_ON_BUS) checkBus(PlayState.child3,numLivingChildren);
			if (!contains(PlayState.child3)) PlayState.child3.die("got lost");
		}
	...
	private function updateBus():Void
	{
		...
		if (car1.body.getLinearVelocity().x != 0) return;
	...
	{
		if (!p.hit.overlaps(busTrigger)) return;
		if (!p.active) return;
		...
		if (p.kind == CHILD)
		{
			...
			if (boarded.length > 0) bus.replaceColor(0xFFFF0000,boarded[0].childColor);
			if (boarded.length > 1) bus.replaceColor(0xFF00FFFF,boarded[1].childColor);
			if (boarded.length > 2) bus.replaceColor(0xFF0000FF,boarded[2].childColor);
		...
		else if (p.kind == PARENT && boarded.length == totalChildren && p.active)
		{
			...
			if (boarded.length > 1) bus.replaceColor(0xFF00FFFF,boarded[1].childColor);
			if (boarded.length > 2) bus.replaceColor(0xFF0000FF,boarded[2].childColor);
			bus.replaceColor(0xFFFFFF00,cast(p,Parent).parentColor);

			if (totalChildren == 0) bus.animation.frameIndex = 7;
			else bus.animation.frameIndex = 3 + totalChildren;
